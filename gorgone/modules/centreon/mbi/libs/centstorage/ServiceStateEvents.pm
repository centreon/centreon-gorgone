	  ...
	  if($logger->{"severity"} eq '0'){
        ...
        if($currentTime%10==0){
		  ...
		  or die "Can't open the following file: $!";
   		  if($speed > 0){
   			my $msg = sprintf("[".localtime(time)."] [LIVE] [SERVICE] Events: %d/%d (%.3f events/sec) - Time Remaining : %d min %d sec ( %.2f %% ) \r",$nbEventTreated,$nbEvents, $speed,(($nbEvents-$nbEventTreated)/$speed)/60,(($nbEvents-$nbEventTreated)/$speed)%60,$nbEventTreated*100/$nbEvents);
   			print $fh $msg;
          }else{
   			my $msg = sprintf("[".localtime(time)."] [LIVE] [SERVICE] Events: %d/%d (n/a events/sec)  ( %.2f %% ) \r",$nbEventTreated,$nbEvents, $nbEventTreated*100/$nbEvents);
  			print $fh $msg;
   		  }
		  close($fh);
        }
